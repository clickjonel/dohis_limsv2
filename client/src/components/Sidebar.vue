<template>
     ...
     <div class="min-w-[15%] h-full flex flex-col justify-start items-center p-2 font-lexend gap-4">
        ...
        <div class="w-full min-h-[80%] flex flex-col justify-start items-center py-4 px-2 gap-4">
            ...
            <Button v-if="store.hasRole('division_chief-approve_ptr') || store.hasRole('property_custodian') || store.hasRole('superadmin')" @click="handleNavigation('Property Transfer Request')" :text="'PTR List'" :buttonType="'navigation'" :icon="'hugeicons:property-new'" :class="route.name === 'Property Transfer Request' ? 'w-full bg-blue-800' : 'w-full' "/>
