<template>
        ...
        <div class="w-full min-h-[85%] flex flex-col justify-start items-center gap-4">
            ...
            <div class="w-full min-h-[10%] flex justify-between items-center border-b">
                ...
                <div class="flex justify-start items-center gap-2 p-2">
                    <Button @click="handleNavigation('/delivery/notice')" text="NOD to COA" buttonType="update" icon="mdi:paper-text-outline" class="translate-y-2.5"/>
                    <Button @click="handleNavigation('/delivery/add')" text="Add Delivery" buttonType="info" icon="material-symbols:add-rounded" class="translate-y-2.5"/>
                </div>
            </div>
    ...
    <!-- <Dialog v-model:visible="showInvalidItemsModal" modal header="This following items are saved with invalid unit cost, please update with valid data." :style="{ width: '80rem',  fontFamily: 'Lexend Deca' }">
        <div class="w-full flex justify-start items-center border-y-2  text-left uppercase py-2 bg-amber-200">
            <span class="min-w-[60%]">Item Description</span>
            <span class="min-w-[20%]">IAR Number</span>
            <span class="min-w-[20%]">Unit Cost</span>
        </div>
        <div v-for="item in invalidItems" class="w-full flex flex-col justify-start items-center gap-2 text-sm">
            <div class="w-full flex justify-between items-start border-b py-2 hover:bg-emerald-500 font-light text-left cursor-pointer" @click="handleNavigation(`/delivery/update/${item.delivery.id}`)">
                <span class="min-w-[60%] px-2" v-html="item.description.replace(/\n/g, '<br>')"></span>
                <span class="min-w-[20%]">{{ item.delivery.iar_no }}</span>
                <span class="min-w-[20%] font-medium text-rose-900 underline animate-pulse">{{ item.unit_cost }}</span>
            </div>
        </div>
    </Dialog> -->
...
<script setup>
    ...
    // var invalidItems = ref([])
    // var showInvalidItemsModal = ref(false)

    var searchKeyword = ref('');

    onMounted(()=>{
        fetchDeliveries()
        // fetchDeliveryItemInvalids()
    })
    ...
    function fetchDeliveries(){
        ...
        .then((response)=>{
            Loading.remove()
            // Notify.success('Deliveries Fetched Successfully',() => {},{fontFamily:'Lexend Deca'})
